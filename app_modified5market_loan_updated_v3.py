...
def schemes_for_business(business_name):
    return [x for x in BUSINESS_SCHEME_MAP.get(business_name, []) if x in SCHEME_DETAILS]
...
if page == "Business Recommendation":
    st.markdown(
        ...
        f'<p>{tr_any("Follow the three steps in order. Inputs start at 0 or blank, and the final advisory appears only after you generate and select a business.")}</p></div>',
        unsafe_allow_html=True,
    )

    # STEP 1
    st.markdown(f'<div class="gs-stepper"><span class="gs-step active">{tr_any("Step 1 • Select Business")}</span><span class="gs-step">{tr_any("Step 2 • Enter Inputs")}</span><span class="gs-step">{tr_any("Step 3 • Strategic Output")}</span></div>', unsafe_allow_html=True)
    st.markdown("## 1️⃣ " + tr_any("Select Business"))
    st.caption(tr_any("Choose the business domain and activity first. The location is taken automatically from the main location selection."))
    ...
    for item in input_items:
        if len(item) < 4 or item[0] in {"water", "irrigation"}:
            continue
    ...
    # STEP 2
    st.markdown("## 2️⃣ " + tr_any("Enter Inputs"))
    st.caption(tr_any("Numeric inputs start at 0 and text inputs start blank. Water availability is automatic from the selected location."))
    ...
    wc3.metric("🌦️ " + tr_any("Rainfall"), f"{LOCATION_CONTEXT.get(location, {}).get('rainfall_mm', 0)} mm")
    ...
    if top3_results:
        st.divider()
        # STEP 3 is shown only after generation.
        st.markdown(f'<div class="gs-stepper"><span class="gs-step active">{tr_any("Step 1 • Business ✓")}</span><span class="gs-step active">{tr_any("Step 2 • Inputs ✓")}</span><span class="gs-step active">{tr_any("Step 3 • Strategic Output")}</span></div>', unsafe_allow_html=True)
        st.markdown("## 3️⃣ " + tr_any("Strategic Output"))
        ...
        if chosen_label:
            ...
            # Every activity gets a related-schemes section.
            related_schemes = schemes_for_business(selected_business)
            st.markdown("### 🏛️ " + tr_any("Related Government Schemes"))
            if related_schemes:
                scheme_rows = []
                for scheme in related_schemes:
                    details = SCHEME_DETAILS.get(scheme, {})
                    scheme_rows.append([scheme, details.get("what_it_is", "See official scheme guidelines."), details.get("benefit", "See official scheme guidelines."), details.get("eligibility", "Eligibility depends on current guidelines.")])
                _md_table(["Scheme", "What it supports", "Financial assistance", "Eligibility"], scheme_rows)
            else:
                st.info(tr_any("Check Government Schemes for current schemes relevant to this activity."))
