...
# ------------------------------------------------------------
# data.gov.in API KEY (configured once, here)
# ------------------------------------------------------------
# The key is kept in the code so the user never has to type it on the
# Market Prices page. Replace the value below with your own data.gov.in
# key if you have one; the default is the public sample key published in
# the data.gov.in API documentation.
MANDI_API_KEY = "579b464db66ec23bdd000001cdd3946e44ce4aad7209ff7b23ac571b"

# Optional override: if a key is placed in .streamlit/secrets.toml as
# MANDI_API_KEY, it is used instead of the value above.
try:
    MANDI_API_KEY = st.secrets.get("MANDI_API_KEY", MANDI_API_KEY) or MANDI_API_KEY
except Exception:
    pass
    ...
    for attempt in range(2):
        try:
            ...
            if "401" in message or "403" in message:
                message = ("The data.gov.in service rejected the configured API key. "
                           "Update MANDI_API_KEY at the top of the Market Prices section "
                           "with your own free key from data.gov.in.")
            return [], message
    return [], last_error


@st.cache_data(ttl=900, show_spinner=False)
def _fetch_karnataka_records(district):
    """
...
def get_location_mandi_prices(location):
    """Latest mandi price for every crop grown/sold at the selected location."""
    district = LOCATION_DISTRICT.get(location, location)
    crops = [c for c in AGRI_LOCATION_CROPS.get(location, [])]
    district_records, state_records, error = _fetch_karnataka_records(district)

    results = []
    for crop in crops:
        record = _best_record(district_records, crop)
        scope = "district"
        if not record:
            record = _best_record(state_records, crop)
            scope = "state"
        if record:
            record["scope"] = scope
        results.append((crop, record))
    return district, results, error
...
if page == "Market Prices":
    ...
    st.write(
        ...
        f"(reported under **{district}** district) from the Government of India "
        f"Open Government Data platform, data.gov.in."
    )

    head_left, head_right = st.columns([4, 1], vertical_alignment="center")
    with head_right:
        if st.button("🔄 Refresh prices", use_container_width=True, key="market_refresh_btn"):
            _fetch_karnataka_records.clear()
            st.rerun()

    crops_here = AGRI_LOCATION_CROPS.get(location, [])
    if not crops_here:
        st.info(
            f"No crop profile is available for {location} yet. "
            "Choose another location from the top bar to see mandi prices."
        )
    else:
        with st.spinner(f"Loading Karnataka mandi prices for {location}..."):
            district, results, error = get_location_mandi_prices(location)
        ...
        st.markdown(
            ...
            f"crops grown or sold around <b>{html.escape(str(location))}</b> • "
            f"Karnataka • prices in ₹ per quintal (100 kg)</div>",
            unsafe_allow_html=True
        )
        ...
        if available:
            ...
            for start in range(0, len(available), per_row):
                ...
                for col, (crop, rec) in zip(cols, chunk):
                    tag_class = "gs-price-tag" if rec.get("scope") == "district" else "gs-price-tag state"
                    tag_text = (
                        f"{html.escape(str(rec['district']))} district"
                        if rec.get("scope") == "district"
                        else "Nearest Karnataka mandi"
                    )
