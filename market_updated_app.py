...
div[data-testid="stHorizontalBlock"]:has([data-testid="stRadio"]) .stSelectbox { margin:0 !important; min-width:125px; }
...
nav_brand, nav_pages, nav_lang, nav_location = st.columns([1.25, 4.8, .85, 1.25], vertical_alignment="center")
with nav_brand:
    st.markdown('<div class="gs-global-brand">🌾 Gram Sahayak</div>', unsafe_allow_html=True)
with nav_pages:
    page = st.radio(
        "Navigate",
        page_values,
...
if page == "Home":
    ...
    <div class="gs-home">
      <div class="gs-hero">
        <div class="gs-hero-copy">
          ...
          <div class="gs-hero-stage">{scene['stage']} • Click below to explore</div>
        </div>
    ...
    st.markdown('<div class="gs-explore-row">', unsafe_allow_html=True)
    if st.button("Explore Gram Sahayak →", key="gs_explore_home"):
        st.session_state.gs_scene = (st.session_state.gs_scene + 1) % len(scenes)
        st.rerun()
    progress = int(((st.session_state.gs_scene + 1) / len(scenes)) * 100)
    st.markdown(f'<div class="gs-progress"><span style="width:{progress}%"></span></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="gs-explore-row">', unsafe_allow_html=True)
    if st.button("See the Next View →", key="gs_start_home"):
        st.session_state.gs_scene = (st.session_state.gs_scene + 1) % len(scenes)
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
...
def _mandi_district_match(record, district):
    wanted = _mandi_norm(district)
    actual = _mandi_norm(record.get("district"))
    aliases = {
        "bengaluru urban": {"bengaluru urban", "bangalore urban", "bengaluru"},
    ...
    return any(x in actual for x in aliases.get(wanted, {wanted}))

@st.cache_data(ttl=900, show_spinner=False)
def _fetch_live_mandi_ui(crop, district, api_key):
    if not api_key:
        return None, "Enter your data.gov.in API key above to load live/latest mandi prices.", False
    if requests is None:
        return None, "The requests package is not installed. Run: pip install requests", False

    # Government mandi data may use a different spelling/name from our UI.
    # Keep all aliases here so the user can select friendly names while the
    # API is queried with the names actually used in mandi records.
    aliases = {
        "Green Chilli": ["Green Chilli", "Green Chilli (Fresh)", "Chilli Green", "Chilli", "Green Chilli Fresh"],
        "Chilli": ["Chilli", "Green Chilli", "Chillies", "Red Chilli"],
        "Lady Finger": ["Lady Finger", "Bhindi", "Okra"],
        "Brinjal": ["Brinjal", "Eggplant", "Brinjal (Purple)"],
        "Coriander": ["Coriander", "Coriander Seed", "Dhaniya"],
        "Groundnut": ["Groundnut", "Ground Nut", "Peanut"],
        "Areca nut": ["Arecanut", "Areca nut", "Areca Nut", "Supari"],
        "Bengal Gram": ["Bengal Gram", "Gram", "Chana", "Kabuli Chana"],
        "Black Gram": ["Black Gram", "Black Gram Dal", "Urad", "Urad Dal"],
        "Green Gram": ["Green Gram", "Green Gram Dal", "Moong", "Moong Dal"],
        "Tur": ["Tur", "Arhar", "Arhar Dal", "Red Gram", "Tur Dal"],
        "Jowar": ["Jowar", "Sorghum"],
        "Bajra": ["Bajra", "Pearl Millet"],
        "Maize": ["Maize", "Corn"],
        "Paddy": ["Paddy", "Paddy(Dhan)(Common)", "Rice"],
    ...
    def _request_records(params):
        ...
        for attempt in range(2):
            try:
                ...
                return payload.get("records", []) if isinstance(payload, dict) else [], None
                ...
                return [], str(exc)
        return [], str(last_exc) if last_exc else None

    records = []
    errors = []
    commodities = aliases.get(crop, [crop])
    districts_to_try = district_aliases.get(district, [district])

    # First try the exact district. If the API uses an older district spelling,
    # try its known alias. This prevents valid records from appearing blank.
    for district_name in districts_to_try:
        for commodity in commodities:
            ...
            got, err = _request_records(params)
            if got:
                records.extend(got)
            if err:
                errors.append(err)
    ...
    if not records:
        if errors and any("timed out" in e.lower() or "timeout" in e.lower() for e in errors):
            return None, "The government mandi server took too long to respond. Please try again in a moment.", False
        return None, f"No live mandi record is currently available for {crop} in {district}.", False

    # The API can return a few records outside the requested spelling. Match
    # the selected district locally before displaying anything.
    matching = [r for r in records if _mandi_district_match(r, district)]
    if not matching:
        return None, f"No live mandi record is currently available for {crop} in {district}.", False
    ...
    for row in matching:
        ...
        return {
            ...
            "modal": md if md is not None else 0,
            "max": mx if mx is not None else 0,
            "commodity": row.get("commodity", crop),
        }, "Live Government of India mandi data loaded.", True

    return None, f"No usable mandi price was returned for {crop} in {district}.", False

# ============================================================
# MARKET PRICES — REQUESTED INTERFACE
# ============================================================
if page == "Market Prices":
    st.title("📈 Market Prices")
    st.write("View the latest available mandi prices from the Government of India's Open Government Data platform.")
    market_api_key = st.text_input(
        "🔑 data.gov.in API key", value="", type="password",
        key="market_ui_api_key",
        help="Enter your data.gov.in API key to load live/latest mandi prices.",
    )
    districts = [
        "Bengaluru Urban", "Belagavi", "Dharwad", "Mysuru", "Shivamogga",
        "Davanagere", "Gadag", "Haveri", "Vijayapura", "Kalaburagi", "Raichur",
        "Tumakuru", "Chitradurga", "Udupi", "Dakshina Kannada", "Mandya", "Hassan",
        "Kodagu", "Kolar", "Chikkaballapur", "Ramanagara", "Chamarajanagar",
        "Bagalkot", "Bidar", "Chikkamagaluru", "Yadgir", "Koppal", "Ballari",
        "Uttara Kannada",
    ]
    district = st.selectbox("📍 District", districts, index=0, key="market_ui_district")
    if not market_api_key.strip():
        st.info("Enter your data.gov.in API key above to load live/latest mandi prices. No sample or simulated prices are shown on this page.")
    else:
        crop_names = list(CROP_DATA.keys())
        selected_crop = st.selectbox("🌾 Crop", crop_names, format_func=tr_data, key="market_ui_crop")
        with st.spinner("Loading latest mandi data..."):
            record, status, ok = _fetch_live_mandi_ui(selected_crop, district, market_api_key.strip())
        if ok and record:
            st.success(status)
            st.caption(f"Market: {record['market']} • District: {record['district']} • Arrival date: {record['date']}")
            c1, c2, c3 = st.columns(3)
            c1.metric("Minimum Price", f"₹{record['min']:,.0f}/quintal")
            c2.metric("Modal Price", f"₹{record['modal']:,.0f}/quintal")
            c3.metric("Maximum Price", f"₹{record['max']:,.0f}/quintal")
        else:
            if "timed out" in status.lower() or "timeout" in status.lower():
                st.warning("The government mandi server took too long to respond. Please try again in a moment.")
                st.caption(status)
            else:
                st.warning(status)
    ...
    st.markdown("<div style='text-align:center; color:#667085; font-size:13px; margin-top:12px;'>Demo market data • Rule-based recommendations • Verify official scheme information before decisions</div>", unsafe_allow_html=True)
    st.stop()
...
if page == "Market Prices":
    ...
    st.write(tr("market_desc"))

    crop_names = list(CROP_DATA.keys())
    selected_crop = st.selectbox("🌾 " + tr("selected_crop"), crop_names, format_func=tr_data)

    base = CROP_DATA[selected_crop]
    ...
    m1.metric(tr("crop"), tr_data(selected_crop))
    m2.metric(tr("sample_price"), f"₹{price:,.0f}/{unit}")
    m3.metric(tr("trend"), tr_data(trend))

    st.markdown(f"### 🌍 {tr('local_suitability')}")
    suitable = AGRI_LOCATION_CROPS.get(location, [])
    # Keep only crops that exist in the market data; location mapping also contains
    # broad categories and crops that are not part of this demo price table.
    suitable_known = [c for c in suitable if c in CROP_DATA]
    if suitable_known:
        st.write(", ".join(tr_data(c) for c in suitable_known))
    else:
        st.info(tr("no_suitability"))

    st.markdown(f"### 📋 {tr('market_board')}")
    rows=[]
    for crop, data in CROP_DATA.items():
        if suitable_known and crop not in suitable_known:
            continue
        rows.append({
            tr("crop"): tr_data(crop),
            tr("sample_price"): f"₹{float(data.get('price',0)):,.0f}/{data.get('unit','kg')}",
            tr("trend"): tr_data(data.get('trend','Stable'))
        })
    if pd is not None:
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    else:
        for row in rows:
            st.write(f"**{row[tr('crop')]}** — {row[tr('sample_price')]} — {row[tr('trend')]}")
