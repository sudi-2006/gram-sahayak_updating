...
def get_crop_image_urls(crops):
    """Fetch crop thumbnails in one request; failure never breaks the market page."""
    ...
    try:
        params = {
            ...
            "piprop": "thumbnail", "pithumbsize": 420, "redirects": 1,
            "titles": "|".join(crops),
        }
        r = requests.get("https://en.wikipedia.org/w/api.php", params=params,
                          headers={"User-Agent": "GramSahayak/1.0"}, timeout=8)
        r.raise_for_status()
        ...
        result = {}
        for crop in crops:
            result[crop] = found.get(crop)
            if result[crop] is None:
                for title, url in found.items():
                    if title.casefold() == crop.casefold():
                        result[crop] = url
                        break
        return result
    except Exception:
