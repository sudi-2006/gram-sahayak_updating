...
<div class="top"><div class="brand"><div class="logo">🎙️</div><div><div class="kicker">Gram Sahayak Intelligence</div><h1>Gram Vaani · AI Voice Assistant</h1></div></div><div class="badge">EN • HI • KN</div></div>
<div class="controls">
<select id="lang"><option value="en-IN">English · India</option><option value="hi-IN">हिन्दी</option><option value="kn-IN">ಕನ್ನಡ</option></select>
<button class="mic" id="mic">🎤 Tap & Speak</button>
<button id="speak">🔊 Read Answer</button>
</div>
<div class="ask"><input id="q" placeholder="Ask about crops, temperature, schemes, loans, markets or business…"><button id="ask">Ask</button></div>
<div class="quick"><button data-q="How does Gram Sahayak work?">How it works</button><button data-q="What is the temperature at my location?">🌡️ Temperature</button><button data-q="Tell me about business recommendation">💡 Business</button><button data-q="Tell me about government schemes">🏛️ Schemes</button><button data-q="Tell me about market prices">📈 Markets</button><button data-q="Explain NPK">🌱 NPK</button></div>
<div class="answer" id="answer"><b>Namaste! / ನಮಸ್ಕಾರ! / Hello!</b><br>I am Gram Vaani. Ask me about farming, markets, schemes, finance, crops, business planning, or your selected location.</div>
<div class="status" id="status">Voice recognition uses your browser's speech service when supported. Microphone permission is required for Tap & Speak.</div>
</div>
<script>
const C={context:{context_json},lang:'en-IN'};
const A={{
...
function speakText(t){{if(!('speechSynthesis' in window)){{status.textContent='Speech synthesis is not supported in this browser.';return}} window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(t);u.lang=C.lang;u.rate=.95;u.pitch=1.02;window.speechSynthesis.speak(u)}}
...
lang.onchange=()=>{{C.lang=lang.value;show(current().hello);status.textContent='Language set to '+lang.options[lang.selectedIndex].text;}};
...
if(!SR){{mic.disabled=true;mic.textContent='🎤 Voice unavailable';status.textContent='Voice recognition is not available in this browser. Text and read-aloud still work.'}}else{{const r=new SR();r.continuous=false;r.interimResults=false;r.maxAlternatives=1;r.onstart=()=>{{mic.classList.add('listening');mic.textContent='⏹ Listening…';status.textContent='Listening in '+lang.options[lang.selectedIndex].text+'…'}};r.onend=()=>{{mic.classList.remove('listening');mic.textContent='🎤 Tap & Speak'}};r.onerror=e=>{{mic.classList.remove('listening');mic.textContent='🎤 Tap & Speak';status.textContent='Microphone error: '+e.error}};r.onresult=e=>{{const text=e.results[0][0].transcript;q.value=text;askNow()}};mic.onclick=()=>{{if(mic.classList.contains('listening')) r.stop(); else {{r.lang=C.lang;r.start()}}}}}}
</script></body></html>
'''
