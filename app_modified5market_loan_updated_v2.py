...
def apply_location_context_to_business_values(domain_name, values, location):
    """Use the globally selected location as the source of water/climate context."""
    out=dict(values)
    ...
    if domain_name=="Agriculture":
        if out.get("temperature") in (None,"") and ctx.get("temperature_c") is not None: out["temperature"]=ctx["temperature_c"]
        if out.get("rainfall") in (None,"") and ctx.get("rainfall_mm") is not None: out["rainfall"]=ctx["rainfall_mm"]
...
def agriculture_preflight_checks(business_name, values, location):
    ...
    if land<=0: issues.append("Land area must be greater than zero.")
    if investment<=0: issues.append("Available investment must be greater than zero.")
...
def show_agriculture_checks(business_name, values, location):
    ...
    if issues:
        st.error("⚠️ Agriculture checks found issues that should be fixed before scaling.")
...
if page == "Business Recommendation":
    st.markdown(
        ...
        f'<p>{tr_any("Select one business activity and enter only the inputs relevant to that activity. The form adapts automatically and missing values are not treated as zero.")}</p></div>',
        unsafe_allow_html=True,
    )
    ...
    with top2:
        business_name = st.selectbox(
            "🧾 " + tr_any("Input profile / starting activity"),
            business_names,
            format_func=tr_any,
            help=tr_any("Choose the closest activity so the app can collect the relevant inputs. The generated shortlist is selected separately below."),
            key="br_business",
        )
    with top3:
        st.markdown(
    ...
    for item in input_items:
        if len(item) < 4:
            continue
        # Water and irrigation are derived from the global location rather than
        # being manually entered again.
        if item[0] in {"water", "irrigation"}:
            continue
    ...
    if domain_name == "Agriculture":
        ...
        for item in AGRICULTURE_ENV_INPUTS:
            if item[0] not in existing and item[0] not in {"water", "irrigation"}:
                groups[_input_group(item[0])].append(item)

    # Location-derived water is shown, not entered.
    auto_water = automatic_location_water(location)
    auto_irrigation = automatic_location_irrigation(location)
    st.markdown(
        f"### 📍 {tr_any('Location & Climate Parameters')}"
    )
    wc1, wc2, wc3 = st.columns(3)
    wc1.metric("💧 " + tr_any("Water availability"), tr_any(auto_water))
    wc2.metric("🚿 Irrigation baseline", tr_any(auto_irrigation))
    wc3.metric("🌦️ Rainfall", f"{LOCATION_CONTEXT.get(location, {}).get('rainfall_mm', '—')} mm")
    st.caption(tr_any("Water availability is automatically derived from the selected location. It is not a manual input."))

    st.markdown(
        f'<div class="gs-stepper"><span class="gs-step active">{tr_any("Step 1 • Select Business")}</span>'
        f'<span class="gs-step active">{tr_any("Step 2 • Enter Inputs")}</span>'
        f'<span class="gs-step">{tr_any("Step 3 • Strategic Output")}</span></div>',
        unsafe_allow_html=True,
    )
    st.divider()
    ...
    with st.expander("🌦️ " + tr_any("Location & Climate Parameters"), expanded=True):
        st.markdown(
            f'<div class="gs-microcopy">{tr_any("Location-derived climate values are planning references. Replace measured soil/climate readings below where available.")}</div>',
            unsafe_allow_html=True,
        )
    ...
    with st.expander("🧪 " + tr_any("Soil & Water Profile"), expanded=True):
        st.markdown(
            f'<div class="gs-microcopy">{tr_any("Water is automatic from location. Soil/NPK fields should use the latest available test or measurement.")}</div>',
            unsafe_allow_html=True,
        )
        card_cols = st.columns(3)
    ...
    if top3_results:
        ...
        st.caption(tr_any("Select the business you actually want to pursue. Execution, advisory, risk and financial guidance will appear only for your selected option."))
        ...
        chosen_label = st.radio(
            "👉 " + tr_any("Select your preferred business option"),
            option_labels,
            index=default_index,
            key="br_option_radio",
        )
        if chosen_label is None:
            st.info("👆 " + tr_any("Select one business option above to view its execution roadmap, advisory, risks and financial plan."))
            selected_business = None
        else:
        ...
        if selected_business is None:
            ...
            if domain_name == "Agriculture":
                ...
                if selected_business in {"Vegetable Cultivation", "Fruit Cultivation"} and ag_issues:
                    st.warning(tr_any("Resolve the highlighted agriculture checks before treating the plan as ready to scale."))

                if ag_env:
                    st.markdown("#### 🌱 Environmental crop check")
                    st.caption(tr_any("Kaggle/NPK matching is a reference check; it is not a guaranteed agronomic recommendation."))
                    env_rows = []
                    for r in ag_env[:12]:
                        env_rows.append([
                            tr_data(r.get("crop", "—")),
                            "Pass" if r.get("valid") else "Check",
                            f"{r.get('score','—')}",
                            r.get("reason", "—"),
                        ])
            ...
            try:
                business_specific_output(
                    selected_business, domain_name, selected_cfg, selected_values,
                    selected_score, selected_reasons, selected_gaps, location,
                )
            except Exception as exc:
