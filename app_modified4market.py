...
# District -> list of talukas/towns available for that district.
DISTRICT_TALUKAS = {}
for _loc in locations:
    DISTRICT_TALUKAS.setdefault(LOCATION_DISTRICT.get(_loc, _loc), []).append(_loc)
for _d in DISTRICT_TALUKAS:
    DISTRICT_TALUKAS[_d] = sorted(DISTRICT_TALUKAS[_d])
districts = sorted(DISTRICT_TALUKAS.keys())
...
if "district" not in st.session_state:
    st.session_state.district = districts[0] if districts else ""
if "location" not in st.session_state:
    st.session_state.location = DISTRICT_TALUKAS.get(st.session_state.district, [""])[0]
if "page" not in st.session_state:
    st.session_state.page = "Home"

nav_brand, nav_pages, nav_lang, nav_district, nav_taluka = st.columns(
    [1.15, 4.2, .8, 1.15, 1.15], vertical_alignment="center"
)
with nav_brand:
...
with nav_district:
    district = st.selectbox(
        "District",
        districts,
        index=districts.index(st.session_state.district) if st.session_state.district in districts else 0,
        format_func=lambda x: "\U0001F4CD " + x,
        label_visibility="collapsed",
        key="gs_top_district"
    )

# The taluka list depends on the chosen district. If the district changed, the
# previously selected taluka no longer belongs to it, so the widget value is
# cleared before the taluka box is drawn.
talukas = DISTRICT_TALUKAS.get(district, [district])
if st.session_state.get("gs_top_taluka") not in talukas:
    st.session_state.pop("gs_top_taluka", None)

with nav_taluka:
    taluka = st.selectbox(
        "Taluka",
        talukas,
        index=talukas.index(st.session_state.location) if st.session_state.location in talukas else 0,
        format_func=lambda x: "\U0001F3D8\uFE0F " + x,
        label_visibility="collapsed",
        key="gs_top_taluka"
    )

# "location" stays the single shared value used by every page in the app.
location = taluka
...
def get_mandi_prices(district, taluka, api_key):
    """
    Latest mandi price for every crop grown or sold in the selected taluka.

    Preference order for each crop:
      1. a mandi inside the selected taluka,
      2. any mandi inside the selected district,
      3. the nearest available mandi elsewhere in Karnataka.
    """
    # Some locations list a broad category ("Vegetables", "Spices") instead of
    # individual crops. Expand those into their constituent crops so each one
    # gets its own price card, the same way every other crop is shown.
    raw_crops = list(AGRI_LOCATION_CROPS.get(taluka, []))
    crops, seen = [], set()
    for crop in raw_crops:
        for expanded in CATEGORY_EXPANSION.get(crop, [crop]):
            if expanded not in seen:
                seen.add(expanded)
                crops.append(expanded)
...
if page == "Market Prices":
    ...
    # District and taluka both come from the top navigation bar, so the same
    # selection is shared by every page of the app.
    taluka = location
    st.write(
        f"Live **Karnataka** mandi prices for **{taluka}** taluka, "
    ...
    crops_here = AGRI_LOCATION_CROPS.get(taluka, [])
    if not market_api_key.strip():
        st.info(
            "Enter your data.gov.in API key above to load live Karnataka mandi prices "
            f"for {taluka} ({district} district). No sample or simulated prices are shown on this page."
        )
    elif not crops_here:
        st.info(
            f"No crop profile is available for {taluka} yet. "
            "Choose another taluka from the top bar to see mandi prices."
        )
    else:
        with st.spinner(f"Loading Karnataka mandi prices for {taluka}, {district}..."):
            results, error = get_mandi_prices(district, taluka, market_api_key.strip())
        ...
        st.markdown(
            ...
            f"crops grown or sold in <b>{html.escape(str(taluka))}</b>, "
            ...
            for start in range(0, len(available), per_row):
                ...
                for col, (crop, rec) in zip(cols, chunk):
                    ...
                    if scope == "taluka":
                        ...
                        tag_text = f"{html.escape(str(taluka))} taluka mandi"
                    elif scope == "district":
                        tag_class = "gs-price-tag"
